...
public abstract class GeometryUtils {
  ...
  public static Line2D[] shapeToLines(Shape path, boolean swapped) {
     return GeometryUtils.shapeToLines(path, swapped, 0.5) ;
  }
   ...
   * @param path Shape to be divided.
   * @param swapped Invert segment direction.
   * @param flatness This function flattens the shape into straight lines - flatness is the 
   * maximum allowable distance between the control points and the flattened curve
   * @return Array of lines.
   */
  public static Line2D[] shapeToLines(Shape path, boolean swapped, double flatness ) {
    Deque<Line2D> lines = new ArrayDeque<Line2D>();
    PathIterator i =
      new FlatteningPathIterator(path.getPathIterator(null), flatness);
    
    int count = 0; 
    
    double[] coords = new double[6];
    double[] coordsPrev = new double[6];
    double[] coordsLastMove = new double[2]; 
    while (!i.isDone()) {
      int segment = i.currentSegment(coords);

      if (segment == PathIterator.SEG_MOVETO) { 
        System.arraycopy(coords, 0, coordsLastMove, 0, 2);
        //System.out.println(count + ": shapeToLines move to " + coordsLastMove[0] + ", " + coordsLastMove[1]);
      }
      ...
      if (segment == PathIterator.SEG_CLOSE && !lines.isEmpty()) {
        Point2D firstPoint = new Point2D.Double(coordsLastMove[0], coordsLastMove[1]); // lines.getFirst().getP1();
